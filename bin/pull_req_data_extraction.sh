...
usage()
{
  ...
  echo "Runs pull_req_data_extraction for a list of projects"
...
parallel --gnu --progress --joblog parjobs --xapply -P $parallel ruby -Ibin pull_req_data_extraction.rb -c config.yaml {1} {2} {3} $dir/{1}@{2}.csv '1>>' $dir/{1}@{2}.csv '2>>' $dir/{1}@{2}.err ::: `cat $input|cut -f1 -d' '` ::: `cat $input|cut -f2 -d' '` &
